...
class ManualTraining:
    ...
    def training(self, training_args, tokenized_train_dataset, tokenized_validation_dataset, data_collator):
        ...
        optimizer = torch.optim.AdamW(
            ...
            lr=getattr(training_args.learning_rate, "learning_rate"),
            weight_decay=getattr(training_args.weight_decay, "weight_decay")
        )

        # Scheduler
        num_epochs = getattr(training_args.num_train_epochs, "num_train_epochs")
        total_steps = len(train_dataloader) * num_epochs
        warmup_steps = getattr(training_args.warmup_steps, "warmup_steps")
        ...
        # Training arguments
        eval_steps = getattr(training_args.eval_steps,"eval_steps")
        save_steps = getattr(training_args.save_steps,"save_steps")
        logging_steps = getattr(training_args.logging_steps, "logging_steps")
        save_total_limit = getattr(training_args.save_total_limit, "save_total_limit")
        load_best_model_at_end = getattr(training_args.load_best_model_at_end, "load_best_model_at_end")
        eval_strategy = getattr(training_args.eval_strategy, "eval_strategy")
